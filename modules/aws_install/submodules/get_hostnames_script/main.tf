...
resource "null_resource" "scp_remote_file_bastion" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      scp -o StrictHostKeyChecking=no -o ProxyCommand="ssh -o StrictHostKeyChecking=no -i ${var.bastion_config.bastion_ssh_key}  ${var.bastion_config.bastion_user}@${var.bastion_config.bastion_host} nc %h %p"   -i ${var.private_key_path} ${var.user}@${var.host_ip}:/tmp/hostnames_output.txt ${path.module}/.
    EOT
  }
